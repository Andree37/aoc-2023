...
bool hand_rank(const std::tuple<std::string, long, long>& q, const std::tuple<std::string, long, long>& p)
{
    if (std::get<2>(p) == std::get<2>(q))
    {
        ...
        for (auto i = 0; i < p_elem.size(); i++)
        {
            ...
            if (auto it = std::find(card_order.begin(), card_order.end(), q_elem[i]); it != card_order.end())
            {
...
bool hand_rank_joker(const std::tuple<std::string, long, long>& q, const std::tuple<std::string, long, long>& p)
{
    if (std::get<2>(p) == std::get<2>(q))
    {
        long p_index = 0;
        ...
        for (auto i = 0; i < p_elem.size(); i++)
        {
            ...
            if (auto it = std::find(card_order_joker.begin(), card_order_joker.end(), q_elem[i]); it != card_order_joker
                .end())
            {
